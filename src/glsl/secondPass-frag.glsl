...
uniform sampler2D u_primaryRayPass;
...
void main() {
		...
		for(float w = 0.0; w <= 1.0; w += 0.2) {
			gl_FragColor += (1.0 - w) * texture2D(u_primaryRayPass, f_uv) + w * texture2D(u_previousFrame, f_uv);
		}
		gl_FragColor /= 1.0 / 0.2; // divide by the number of samples (1 / stepSize)
	#else
		gl_FragColor = texture2D(u_primaryRayPass, f_uv);
	#endif
}
